package com.busy.beefly.game.actors
...
import com.busy.beefly.game.actors.checkbox.gdxGame
import com.busy.beefly.game.utils.TextureEmpty
...
open class AButton(
    ...
    fun setOnClickListener(sound: Sound? = gdxGame.soundUtil.DOWN, block: () -> Unit) {
        this.sound   = sound
        onClickBlock = block
    }
    ...
    private fun getStyleByType(type: Type) = when(type) {
        ...
        Type.Rules -> AButtonStyle(
            default  = TextureRegionDrawable(gdxGame.allAssets.rl_def),
            pressed  = TextureRegionDrawable(gdxGame.allAssets.rl_prs),
            disabled = TextureRegionDrawable(gdxGame.allAssets.rl_prs),
        )
    }
