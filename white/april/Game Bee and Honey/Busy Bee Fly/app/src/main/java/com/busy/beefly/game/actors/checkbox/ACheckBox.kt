package com.busy.beefly.game.actors.checkbox
...
open class ACheckBox(
        ...
        override fun touchUp(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int) {
            ...
            if (isWithin) {
                if (touchPointDown.x in (touchPointUp.x - radiusTouch..touchPointUp.x + radiusTouch) &&
                    ...
                    gdxGame.soundUtil.apply { play(DOWN) }
    ...
    fun getStyleByType(type: Type) = when(type) {
        Type.MUS -> ACheckBoxStyle(
            default = gdxGame.allAssets.mus_on.region,
            checked = gdxGame.allAssets.mus_off.region,
        )
    }
