package com.epicjourney.seredmora.game.actors.button
...
open class AButton(
    ...
    private fun getListener() = object : InputListener() {
        ...
        override fun touchDown(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int): Boolean {
            ...
            screen.game.soundUtil.apply { play(BTN_CLICK) }
    ...
    private fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.VAL_MENU -> Static.AButtonStyle(
            default = assets.val_menu_def,
            pressed = assets.val_menu_press,
            disabled = assets.val_menu_press,
        )
    }
