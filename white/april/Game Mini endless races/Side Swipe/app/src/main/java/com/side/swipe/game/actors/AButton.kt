package com.side.swipe.game.actors
...
//            screen.game.soundUtil.apply { play(sound ?: click) }
    ...
    private fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.LEFT  -> Static.AButtonStyle(
            default = spriteUtil.left_def,
            pressed = spriteUtil.left_press,
            disabled = spriteUtil.left_press,
        )
        Static.Type.RIGHT  -> Static.AButtonStyle(
            default = spriteUtil.right_def,
            pressed = spriteUtil.right_press,
            disabled = spriteUtil.right_press,
        )
    }
