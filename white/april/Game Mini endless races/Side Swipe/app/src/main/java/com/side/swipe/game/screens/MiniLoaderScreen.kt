...
class MiniLoaderScreen(override val game: LibGDXGame) : AdvancedScreen() {
    ...
    private fun loadSplashAssets() {
        with(game.spriteManager) {
            loadableTextureList = mutableListOf(SpriteManager.EnumTexture.mini.data)
            loadTexture()
        }
    ...
    private fun isFinish() {
        if (isFinishProgress) {
            ...
            game.musicUtil.apply {
                coff  = 0.279f
                music = passion.apply { isLooping = true }
            }

            stageUI.root.animHide(TIME_ANIM) {
                game.navigationManager.navigate(MiniMenuScreen::class.java.name)
            }
        }
    }
