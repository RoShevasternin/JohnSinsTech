package com.warehousev.operatorw.game.actors.checkbox
...
import com.smallmini.races.gonca.game.actors.checkbox.ACheckBoxGroup
...
open class ACheckBox(
    ...
    fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.MUSIC -> Static.ACheckBoxStyle(
            default = assets.mus_on.region,
            checked = assets.mus_off.region,
        )
        Static.Type.SOUND -> Static.ACheckBoxStyle(
            default = assets.sou_on.region,
            checked = assets.sou_off.region,
        )
    }
